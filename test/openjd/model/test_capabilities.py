...
def _success_test_values(prefix: str) -> list:
    return (
        [
            ...
            pytest.param(FormatString(f"{prefix}.worker.foo", context=ModelParsingContext_v2023_09()), id="format string no expression"),
            pytest.param(
                FormatString(f"{prefix.upper()}.WORKER.FOO", context=ModelParsingContext_v2023_09()), id="caps format string no expression"
            ),
            pytest.param(FormatString("{{ Param.Foo }}", context=ModelParsingContext_v2023_09()), id="format string with expression"),
            pytest.param(
                FormatString(f"{prefix}.{{{{ Param.Foo }}}}", context=ModelParsingContext_v2023_09()), id="format string partial expression"
            ),
        ]
