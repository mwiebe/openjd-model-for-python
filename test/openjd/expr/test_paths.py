...
class TestPaths:
    ...
    def test_path_parent(self, tmp_path) -> None:
        p = tmp_path / "projects" / "shot01" / "render.exr"
        symtab = SymbolTable({"P": _p(p)})
        assert evaluate_expression("P.parent", values=symtab).to_string() == str(
    ...
    def test_path_join(self, tmp_path) -> None:
        ...
        assert result.to_string() == str(d / "subdir" / "file.txt")

    def test_path_concat(self, tmp_path) -> None:
        p = tmp_path / "output" / "file"
        symtab = SymbolTable({"P": _p(p)})
        assert evaluate_expression("P + '.txt'", values=symtab).to_string() == str(p) + ".txt"

    def test_with_suffix(self, tmp_path) -> None:
        p = tmp_path / "output" / "render.exr"
        symtab = SymbolTable({"P": _p(p)})
        assert (
            evaluate_expression("with_suffix(P, '.png')", values=symtab)
            .to_string()
            .endswith("render.png")
        )
    ...
    def test_path_from_string(self) -> None:
        assert (
            evaluate_expression("path('/tmp/test')", path_format=PathFormat.POSIX).to_string()
            == "/tmp/test"
        )

    def test_path_from_list(self) -> None:
        result = evaluate_expression('path(["a", "b", "c"])').to_string()
    ...
    def test_path_from_sliced_parts(self, tmp_path) -> None:
        ...
        result = evaluate_expression("path(P.parts[:3])", values=symtab).to_string()
        expected = str(PurePath(*p.parts[:3]))
        assert result == expected

    def test_path_from_parts_skip_root(self) -> None:
        # Skip root, get relative path
        result = evaluate_expression('path(path("/a/b/c").parts[1:])').to_string()
        assert result == str(PurePath("a", "b", "c"))

    def test_path_from_parts_last_two(self) -> None:
        result = evaluate_expression('path(path("/a/b/c/d").parts[-2:])').to_string()
...
class TestRelativeTo:
    ...
    def test_uri_basic(self) -> None:
        ...
        assert result.to_string() == "file.txt"
    ...
    def test_unc_basic(self) -> None:
        ...
        assert result.to_string() == "dir\\file"
    ...
    def test_unc_not_relative(self) -> None:
        ...
        assert str(exc_info.value) == "".join(
            [
                r"relative_to failed: '\\server\share\dir' is not relative to '\\other\share\'"
                + "\n",
                "  path('//server/share/dir').relative_to(path('//other/share'))\n",
                "  ~~~~~~~~~~~~~~~~~~~~~~~~~~~^~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~",
            ]
...
class TestPropertyAccessOnFunctionResults:
    ...
    def test_path_parent_on_function_result(self) -> None:
        result = evaluate_expression('path("/a/b/c.txt").parent').to_string()
    ...
    def test_repeated_parent_access(self, tmp_path) -> None:
        ...
        assert evaluate_expression("P.parent.parent", values=symtab).to_string() == str(
            tmp_path / "a" / "b" / "c"
        )
        assert evaluate_expression("P.parent.parent.parent", values=symtab).to_string() == str(
            tmp_path / "a" / "b"
        )
...
class TestWithNumber:
    ...
    def test_digit_sequence(self, tmp_path) -> None:
        ...
        assert result.to_string() == str(tmp_path / "renders" / "shot_072.exr")

    def test_printf_04d(self, tmp_path) -> None:
        p = tmp_path / "renders" / "shot_%04d.exr"
        symtab = SymbolTable({"P": _p(p)})
        result = evaluate_expression("P.with_number(72)", values=symtab)
        assert result.to_string() == str(tmp_path / "renders" / "shot_0072.exr")

    def test_printf_d(self, tmp_path) -> None:
        p = tmp_path / "renders" / "shot_%d.exr"
        symtab = SymbolTable({"P": _p(p)})
        result = evaluate_expression("P.with_number(72)", values=symtab)
        assert result.to_string() == str(tmp_path / "renders" / "shot_72.exr")
    ...
    def test_hash_6(self, tmp_path) -> None:
        ...
        symtab = SymbolTable({"P": _p(p)})
        result = evaluate_expression("P.with_number(72)", values=symtab)
        assert result.to_string() == str(tmp_path / "renders" / "shot_000072.exr")
    ...
    def test_multi_extension_vfx(self, tmp_path) -> None:
        ...
        symtab = SymbolTable({"P": _p(p)})
        result = evaluate_expression("P.with_number(72)", values=symtab)
        assert result.to_string() == str(tmp_path / "renders" / "render.0072.exr")
    ...
    def test_multi_extension_version(self, tmp_path) -> None:
        ...
        symtab = SymbolTable({"P": _p(p)})
        result = evaluate_expression("P.with_number(72)", values=symtab)
        assert result.to_string() == str(tmp_path / "renders" / "file.v2.072.exr")
    ...
    def test_digits_as_extension(self, tmp_path) -> None:
        ...
        symtab = SymbolTable({"P": _p(p)})
        result = evaluate_expression("P.with_number(72)", values=symtab)
        assert result.to_string() == str(tmp_path / "renders" / "file_0072.001")
    ...
    def test_mixed_printf_and_hash_rightmost_wins(self, tmp_path) -> None:
        ...
        assert result.to_string() == str(tmp_path / "renders" / "f_%d_abcdefg_042.exr")

    def test_mixed_printf_and_digits_rightmost_wins(self, tmp_path) -> None:
        p = tmp_path / "renders" / "file_%04d_003.exr"
        symtab = SymbolTable({"P": _p(p)})
        result = evaluate_expression("P.with_number(72)", values=symtab)
        assert result.to_string() == str(tmp_path / "renders" / "file_%04d_072.exr")
