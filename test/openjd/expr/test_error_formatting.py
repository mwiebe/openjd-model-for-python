...
class TestErrorCaretPointers:
        ...
        symtab = SymbolTable(
            {
                "Param": SymbolTable(
                    {
                        "List": ExprValue._from_list(
                            [ExprValue(i) for i in [1, 2, 3]],
                            ExprValue(0).type,
                        ),
                    }
                )
    ...
    def test_undefined_variable(self):
        ...
        expected = [
            "Undefined variable: Param.InputFiel\n",
            "  Param.InputFiel + 1\n",
            "  ~~~~~~^~~~~~~~~",
        ]
        assert str(exc_info.value) == "".join(expected)
...
class TestSyntaxErrorCarets:
    ...
    def test_unclosed_paren(self):
        """Unclosed parenthesis shows caret at opening paren."""
        with pytest.raises(ExpressionError) as exc_info:
    ...
    def test_unclosed_string(self):
        """Unclosed string shows caret at string start."""
        with pytest.raises(ExpressionError) as exc_info:
