...
from openjd.expr._errors import ExpressionTypeError
...
def _eval_with_target(expr: str, target_type: ExprType | None, path_format=None):
    ...
    evaluator = Evaluator([SymbolTable({})], get_default_library(), path_format=path_format)
...
class TestListLiteralTypeInference:
    ...
    def test_nested_int_float_coerces_values(self) -> None:
        ...
        result = evaluate_expression("[[1], [2.0]]")
        inner_first = result.to_expr_value_list()[0]
    ...
    def test_null_in_list_fails(self) -> None:
        with pytest.raises(ExpressionTypeError, match="null.*cannot be an element"):
    ...
    def test_int_string_fails(self) -> None:
        with pytest.raises(ExpressionTypeError, match="incompatible types.*int.*string"):
            evaluate_expression("[1, 'a']")

    def test_int_bool_fails(self) -> None:
        with pytest.raises(ExpressionTypeError, match="incompatible types.*int.*bool"):
            evaluate_expression("[1, True]")

    def test_string_bool_fails(self) -> None:
        with pytest.raises(ExpressionTypeError, match="incompatible types.*string.*bool"):
            evaluate_expression('["a", True]')

    def test_scalar_list_fails(self) -> None:
        with pytest.raises(ExpressionTypeError, match="incompatible types.*int.*list"):
            evaluate_expression("[1, [2]]")

    def test_three_incompatible_types_fails(self) -> None:
        with pytest.raises(ExpressionTypeError, match="incompatible types.*int.*float.*string"):
            evaluate_expression('[1, 2.0, "a"]')

    def test_path_int_fails(self) -> None:
        with pytest.raises(ExpressionTypeError, match="incompatible types.*path.*int"):
            evaluate_expression('[path("/a"), 1]')

    def test_three_level_nesting_fails(self) -> None:
        with pytest.raises(ExpressionTypeError, match="cannot be nested more than 2 levels"):
            evaluate_expression("[[[1, 2]]]")
...
class TestListElementCoercion:
    ...
    def test_int_to_string(self) -> None:
        result = _eval_with_target("[1, 2, 3]", ExprType(TypeCode.LIST, [STRING]))
        assert str(result.type) == "list[string]"
        assert result.item() == ["1", "2", "3"]

    def test_float_to_string(self) -> None:
        result = _eval_with_target("[1.5, 2.0]", ExprType(TypeCode.LIST, [STRING]))
        assert str(result.type) == "list[string]"
        assert result.item() == ["1.5", "2.0"]

    def test_bool_to_string(self) -> None:
        result = _eval_with_target("[True, False]", ExprType(TypeCode.LIST, [STRING]))
        assert str(result.type) == "list[string]"
    ...
    def test_float_to_int_exact(self) -> None:
        result = _eval_with_target("[1.0, 2.0, 3.0]", ExprType(TypeCode.LIST, [INT]))
        assert str(result.type) == "list[int]"
        assert result.item() == [1, 2, 3]

    def test_float_to_int_not_exact(self) -> None:
        with pytest.raises(ExpressionTypeError, match="not a whole number"):
            _eval_with_target("[1.5]", ExprType(TypeCode.LIST, [INT]))

    def test_string_to_int_valid(self) -> None:
        result = _eval_with_target('["1", "2", "3"]', ExprType(TypeCode.LIST, [INT]))
        assert str(result.type) == "list[int]"
        assert result.item() == [1, 2, 3]

    def test_string_to_int_invalid(self) -> None:
        with pytest.raises(ExpressionTypeError, match="Cannot coerce"):
            _eval_with_target('["abc"]', ExprType(TypeCode.LIST, [INT]))

    def test_string_to_int_float_string(self) -> None:
        with pytest.raises(ExpressionTypeError, match="Cannot coerce"):
            _eval_with_target('["3.1"]', ExprType(TypeCode.LIST, [INT]))

    # -> list[float]
    def test_int_to_float(self) -> None:
        result = _eval_with_target("[1, 2, 3]", ExprType(TypeCode.LIST, [FLOAT]))
        assert str(result.type) == "list[float]"
        assert result.item() == [1.0, 2.0, 3.0]

    def test_string_to_float_valid(self) -> None:
        result = _eval_with_target('["1.5", "2.0"]', ExprType(TypeCode.LIST, [FLOAT]))
        assert str(result.type) == "list[float]"
        assert result.item() == [1.5, 2.0]

    def test_string_to_float_invalid(self) -> None:
        with pytest.raises(ExpressionTypeError, match="Cannot coerce"):
            _eval_with_target('["abc"]', ExprType(TypeCode.LIST, [FLOAT]))

    # bool cannot coerce to int/float/path
    def test_bool_to_int_fails(self) -> None:
        with pytest.raises(ExpressionTypeError, match="Cannot coerce"):
            _eval_with_target("[True]", ExprType(TypeCode.LIST, [INT]))

    def test_bool_to_float_fails(self) -> None:
        with pytest.raises(ExpressionTypeError, match="Cannot coerce"):
            _eval_with_target("[False]", ExprType(TypeCode.LIST, [FLOAT]))

    def test_bool_to_path_fails(self) -> None:
        with pytest.raises(ExpressionTypeError, match="Cannot coerce"):
            _eval_with_target("[True]", ExprType(TypeCode.LIST, [PATH]))

    # Mixed types in list with coercion
    def test_mixed_int_string_to_string(self) -> None:
        result = _eval_with_target('["--quality", 5]', ExprType(TypeCode.LIST, [STRING]))
        assert str(result.type) == "list[string]"
...
class TestNestedListElementCoercion:
    ...
    def test_nested_mixed_to_string(self) -> None:
        list_list_string = ExprType(TypeCode.LIST, [ExprType(TypeCode.LIST, [STRING])])
        result = _eval_with_target(
    ...
    def test_nested_string_to_path(self) -> None:
        list_list_path = ExprType(TypeCode.LIST, [ExprType(TypeCode.LIST, [PATH])])
    ...
    def test_nested_mixed_to_int(self) -> None:
        list_list_int = ExprType(TypeCode.LIST, [ExprType(TypeCode.LIST, [INT])])
    ...
    def test_nested_mixed_to_float(self) -> None:
        list_list_float = ExprType(TypeCode.LIST, [ExprType(TypeCode.LIST, [FLOAT])])
...
class TestListConcatenation:
    ...
    def test_concat_symtab_range_expr(self) -> None:
        ...
        symtab = SymbolTable({"frames": IntRangeExpr.from_str("1-5"), "extra": [100, 200]})
...
class TestRange:
    ...
    def test_range_step_zero_error(self) -> None:
        with pytest.raises(ExpressionError, match="step argument must not be zero"):
