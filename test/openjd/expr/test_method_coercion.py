...
import pytest
from openjd.expr import evaluate_expression, ExpressionTypeError
...
class TestMethodCallNoReceiverCoercion:
    ...
    def test_path_startswith_as_method_fails(self) -> None:
        ...
        with pytest.raises(ExpressionTypeError, match="startswith\\(\\) is not available for path"):
    ...
    def test_path_endswith_as_method_fails(self) -> None:
        ...
        with pytest.raises(ExpressionTypeError, match="endswith\\(\\) is not available for path"):
    ...
    def test_path_split_as_method_fails(self) -> None:
        ...
        with pytest.raises(ExpressionTypeError, match="split\\(\\) is not available for path"):
