...
from openjd.expr._types import ExprType
...
class TestPathFormatMismatch:
    ...
    def test_matching_format_no_error(self) -> None:
        ...
        assert result.to_string() == "/tmp/render.exr"

    def test_create_path_without_path_format_raises(self) -> None:
        """_create rejects PATH ExprValues without path_format."""
        with pytest.raises(ExpressionTypeError, match="requires path_format"):
            ExprValue._create(ExprType.PATH, string_value="/tmp/test")
    ...
    def test_evaluator_defaults_to_host_format(self) -> None:
        ...
        assert result.to_string() == val.to_string()
