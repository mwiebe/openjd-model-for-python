...
class TestExprTypeStringConstruction:
    ...
    def test_list_types(self) -> None:
        assert ExprType("list[int]") == ExprType.LIST_INT
        assert ExprType("list[string]") == ExprType.LIST_STRING
        assert ExprType("list[float]") == ExprType.LIST_FLOAT
        assert ExprType("list[path]") == ExprType.LIST_PATH
        assert ExprType("list[bool]") == ExprType.LIST_BOOL

    def test_nested_list(self) -> None:
        assert ExprType("list[list[int]]") == ExprType.LIST_LIST_INT
...
class TestUnionTypes:
    ...
    def test_any_matches_anything(self) -> None:
        any_type = ExprType(TypeCode.ANY)
        assert any_type.match(ExprType.INT) == {}
        assert any_type.match(ExprType.STRING) == {}
        assert any_type.match(ExprType.LIST_INT) == {}

    def test_anything_matches_any(self) -> None:
        any_type = ExprType(TypeCode.ANY)
        assert ExprType.INT.match(any_type) == {}
        assert ExprType.STRING.match(any_type) == {}

    def test_is_concrete(self) -> None:
        from openjd.expr._types import T1

        # Concrete types
        assert ExprType.INT.is_concrete()
        assert ExprType.STRING.is_concrete()
        assert ExprType.LIST_INT.is_concrete()
    ...
    def test_union_matches_member(self) -> None:
        union = ExprType(TypeCode.UNION, [ExprType.INT, ExprType.STRING])
        assert union.match(ExprType.INT) == {}
        assert union.match(ExprType.STRING) == {}
        assert union.match(ExprType.FLOAT) is None

    def test_member_matches_union(self) -> None:
        union = ExprType(TypeCode.UNION, [ExprType.INT, ExprType.STRING])
        assert ExprType.INT.match(union) == {}
        assert ExprType.STRING.match(union) == {}
        assert ExprType.FLOAT.match(union) is None
    ...
    def test_typevar_binds_to_union(self) -> None:
        ...
        assert T1.match(union) == {TypeCode.TYPEVAR_T1: union}
        assert union.match(T1) == {TypeCode.TYPEVAR_T1: union}
    ...
    def test_nested_typevar_binds_to_union(self) -> None:
        ...
        result = list_t1.match(list_union)
    ...
    def test_any_with_typevar(self) -> None:
        ...
        assert T1.match(any_type) == {TypeCode.TYPEVAR_T1: any_type}
        assert any_type.match(T1) == {TypeCode.TYPEVAR_T1: any_type}
...
class TestExprValue:
    ...
    def test_from_path(self) -> None:
        ...
        assert val.to_string() == "/tmp/test"

    def test_from_list(self) -> None:
        # GIVEN
        items = [ExprValue(1), ExprValue(2)]

        # WHEN
        val = ExprValue._from_list(items, INT)

        # THEN
        assert val.type.type_code == TypeCode.LIST
        assert val.item() == [1, 2]
    ...
    def test_to_string_bool(self) -> None:
        assert ExprValue(True).to_string() == "true"
        assert ExprValue(False).to_string() == "false"

    def test_to_string_int(self) -> None:
        assert ExprValue(42).to_string() == "42"
        assert ExprValue(-5).to_string() == "-5"

    def test_to_string_float(self) -> None:
        assert ExprValue(3.14).to_string() == "3.14"

    def test_to_string_null(self) -> None:
        assert ExprValue.null().to_string() == ""
    ...
    def test_direct_construction(self) -> None:
        """Test ExprValue(python_value) construction."""
        from openjd.model import IntRangeExpr

        assert ExprValue(42).type == INT
        assert ExprValue(3.14).type == FLOAT
        assert ExprValue("hello").type == ExprType.STRING
        assert ExprValue(True).type == BOOL
        assert ExprValue(None).is_null
        assert ExprValue([1, 2, 3]).type == ExprType.LIST_INT
        assert ExprValue(IntRangeExpr("1-10")).type == ExprType.RANGE_EXPR

    def test_item_extraction(self) -> None:
        """Test .item() method."""
        from openjd.model import IntRangeExpr

        assert ExprValue(42).item() == 42
        assert ExprValue(3.14).item() == 3.14
        assert ExprValue("hello").item() == "hello"
        assert ExprValue(True).item() is True
        assert ExprValue(None).item() is None
        assert ExprValue([1, 2, 3]).item() == [1, 2, 3]
        assert ExprValue("/tmp", type="path", path_format=PathFormat.POSIX).item() == "/tmp"
        r = IntRangeExpr("1-5")
        assert ExprValue(r).item() is r
    ...
    def test_list_nested_int_float_mix_coerces_values(self) -> None:
        ...
        v = ExprValue([[1], [2.0]])
        inner_first = v.to_expr_value_list()[0]
        assert str(inner_first.type) == "list[float]"
        assert isinstance(inner_first.to_expr_value_list()[0].item(), float)
    ...
    def test_list_nested_path_string_mix_coerces_values(self) -> None:
        ...
        assert isinstance(inner_first.to_expr_value_list()[0].item(), str)

    def test_list_incompatible_types_raises(self) -> None:
        """Incompatible element types raise TypeError."""
    ...
    def test_type_coercion(self) -> None:
        ...
        assert ExprValue(["1", "2"], type=ExprType.LIST_INT).item() == [1, 2]
...
class TestTypeVariables:
    ...
    def test_match_simple(self) -> None:
        ...
        bindings = T1.match(ExprType.INT)
    ...
    def test_match_nested(self) -> None:
        ...
        list_t1 = ExprType(TypeCode.LIST, [T1])
        list_int = ExprType(TypeCode.LIST, [ExprType.INT])
        bindings = list_t1.match(list_int)
        assert bindings == {TypeCode.TYPEVAR_T1: ExprType.INT}
    ...
    def test_match_no_match(self) -> None:
        ...
        list_t1 = ExprType(TypeCode.LIST, [T1])
        assert list_t1.match(ExprType.INT) is None  # Not a list
        assert list_t1.match(ExprType.STRING) is None  # Not a list
    ...
    def test_expression_value_rejects_non_concrete_type(self) -> None:
        from openjd.expr._types import T1
        from openjd.expr import ExprValue, ExpressionTypeError

        # Symbolic type
        with pytest.raises(ExpressionTypeError, match="must have concrete type"):
            ExprValue._create(T1, int_value=42)

        # ANY type
        with pytest.raises(ExpressionTypeError, match="must have concrete type"):
            ExprValue._create(ExprType(TypeCode.ANY), int_value=42)

        # UNION type
        with pytest.raises(ExpressionTypeError, match="must have concrete type"):
            ExprValue._create(
                ExprType(TypeCode.UNION, [ExprType.INT, ExprType.STRING]), int_value=42
            )
...
class TestUnknownType:
    ...
    def test_construct_requires_exactly_one_param(self) -> None:
    ...
    def test_parse_unknown_list(self) -> None:
        ...
        assert t.type_code == TypeCode.UNRESOLVED
        assert t.type_params == [ExprType.LIST_STRING]
    ...
    def test_match_delegates_to_constraint(self) -> None:
        t = ExprType("unresolved[int]")
        assert t.match(ExprType.INT) == {}
        assert t.match(ExprType.STRING) is None
        assert t.match(ExprType.FLOAT) is None

    def test_match_symmetric(self) -> None:
        t = ExprType("unresolved[int]")
        assert ExprType.INT.match(t) == {}
        assert ExprType.STRING.match(t) is None

    def test_match_union_constraint(self) -> None:
        t = ExprType("unresolved[int | float]")
        assert t.match(ExprType.INT) == {}
        assert t.match(ExprType.FLOAT) == {}
        assert t.match(ExprType.STRING) is None

    def test_match_any_constraint(self) -> None:
        t = ExprType("unresolved")
        assert t.match(ExprType.INT) == {}
        assert t.match(ExprType.STRING) == {}
        assert t.match(ExprType.LIST_INT) == {}

    def test_match_unknown_vs_unknown(self) -> None:
        # unresolved[int] vs unresolved[int] - delegates to int.match(unresolved[int]) -> int.match(int)
    ...
    def test_match_unknown_vs_any(self) -> None:
        t = ExprType("unresolved[int]")
        assert t.match(ExprType(TypeCode.ANY)) == {}
        assert ExprType(TypeCode.ANY).match(t) == {}
    ...
    def test_exprvalue_unknown_equality(self) -> None:
        ...
        assert ExprValue.unresolved(ExprType.INT) != ExprValue.null()

    def test_exprvalue_unknown_to_string_raises(self) -> None:
        from openjd.expr import ExpressionTypeError

        with pytest.raises(ExpressionTypeError, match="value is not known"):
            ExprValue.unresolved(ExprType.INT).to_string()

    def test_exprvalue_unknown_item_raises(self) -> None:
        from openjd.expr import ExpressionTypeError
    ...
    def test_exprvalue_list_rejects_unknown_elements(self) -> None:
        with pytest.raises(TypeError, match="Cannot construct a list containing unresolved values"):
