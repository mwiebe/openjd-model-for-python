...
class TestStrings:
    ...
    def test_split_empty_string(self) -> None:
        ...
        assert [v.item() for v in r.to_expr_value_list()] == [""]
...
class TestReprFunctions:
    ...
    def test_repr_py_range_expr(self) -> None:
        ...
        symtab = SymbolTable({"Frames": IntRangeExpr.from_str("1-5")})
...
class TestReprPwshComprehensive:
    ...
    def test_pwsh_range_expr_with_step(self) -> None:
        ...
        symtab = SymbolTable({"Frames": IntRangeExpr.from_str("1-10:2")})
    ...
    def test_pwsh_range_expr_as_list(self) -> None:
        ...
        symtab = SymbolTable({"Frames": IntRangeExpr.from_str("1-3")})
...
class TestReprCmdComprehensive:
    ...
    def test_cmd_string_carriage_return(self) -> None:
        assert evaluate_expression(r"repr_cmd('a\rb')").item() == '"a\rb"'
...
class TestReprCmdWindowsValidation:
    ...
    def test_cmd_string_newline_roundtrip(self) -> None:
        """Verify newlines trigger quoting. CMD cannot truly handle embedded
        newlines in arguments, but quoting prevents the newline from being
        interpreted as a command separator."""
        # Without the fix, repr_cmd('a\nb') would return a\nb unquoted,
        # which in a .bat file would execute 'b' as a separate command.
        quoted = evaluate_expression(r"repr_cmd('a\nb')").item()
        assert quoted.startswith('"'), f"Expected quoted string, got: {quoted!r}"
        # Verify the unquoted version would NOT be quoted (baseline)
        unquoted = evaluate_expression("repr_cmd('ab')").item()
        assert not unquoted.startswith('"'), f"Expected unquoted string, got: {unquoted!r}"
