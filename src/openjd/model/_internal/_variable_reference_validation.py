...
def prevalidate_model_template_variable_references(cls: Type[OpenJDModel], values: dict[str, Any], *, context: Optional[ModelParsingContextInterface]):
    ...
    *,
            ...
            for i, item in enumerate(value):
                errors.extend(
                    _validate_model_template_variable_references(
                        item_model, item, current_scope, symbol_prefix, symbols, (*loc, i), context=context
                    )
                )
        return errors
    ...
    if isclass(model) and lenient_issubclass(model, FormatString):
        if not isinstance(value, FormatString):
            if context is None:
                raise ValueError(f"Internal parsing error: value {value!r} type {type(value)} should be a FormatString")
            print(f"FormatString is {FormatString}, context is {context}")
...
def _check_format_string(
    value: FormatString, current_scope: ResolutionScope, symbols: ScopedSymtabs, loc: tuple,
    context: Optional[ModelParsingContextInterface]
