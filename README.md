...
When you have an Open Job Description model object, like the job template decoded in the previous
section, you can convert it into an object suitable for converting to JSON or YAML
by using the `model_to_object` function.
...
To call `parse_model`, you will need to prepare a `ModelParsingContext` to use for building
the individual parts. Individual model objects can accept inputs differently depending on
what extensions are requested in the job template, and the model parsing context stores what
they need. The functions `decode_job_template` and `decode_environment_template` create this
context from top-level template fields, but `parse_model` doesn't have the same context so you
...
You can also use the `openjd.model.parse_model` function to decode individual parts of the template.
