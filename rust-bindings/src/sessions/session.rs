...
use openjd_model::types::{JobParameterValue, JobParameterType, JobParameterValues};
use openjd_model::types::{TaskParameterValue, TaskParameterType, TaskParameterSet};
...
use super::types::{PySessionState, PyActionStatus};
    ...
    for (key, val) in py_dict.iter() {
        ...
        if let Ok(inner_dict) = val.cast::<PyDict>() {
            ...
            let param_type = JobParameterType::from_spec_str(&type_str).ok_or_else(|| {
                pyo3::exceptions::PyValueError::new_err(format!("Unknown parameter type: {type_str}"))
            })?;
    ...
    for (key, val) in py_dict.iter() {
        ...
        if let Ok(inner_dict) = val.cast::<PyDict>() {
            ...
            let param_type = TaskParameterType::from_spec_str(&type_str).ok_or_else(|| {
                pyo3::exceptions::PyValueError::new_err(format!("Unknown task parameter type: {type_str}"))
            })?;
            let value_obj = inner_dict
                .get_item("value")?
                .ok_or_else(|| pyo3::exceptions::PyKeyError::new_err("Missing 'value' key"))?;
...
impl PySession {
    ...
    fn action_status(&self) -> Option<PyActionStatus> {
        self.snapshot.lock().unwrap().action_status.clone().map(PyActionStatus::from)
    }
    ...
    fn enter_environment(
        ...
        std::thread::spawn(move || {
            ...
            let _ = rt.block_on(
                session.enter_environment(&env, None, Some(&env_id), env_ref)
            );
    ...
    fn exit_environment(
        ...
        std::thread::spawn(move || {
            ...
            let env_ref = os_env_vars.as_ref();
            let _ = rt.block_on(
                session.exit_environment(&identifier, None, keep_session_running, env_ref)
            );
            *session_arc.lock().unwrap() = Some(session);
            let guard = session_arc.lock().unwrap();
            if let Some(s) = guard.as_ref() {
    ...
    fn run_task(
        ...
        std::thread::spawn(move || {
            ...
            let _ = rt.block_on(
                session.run_task(&script, task_ref, None, env_ref)
            );
            *session_arc.lock().unwrap() = Some(session);
            let guard = session_arc.lock().unwrap();
            if let Some(s) = guard.as_ref() {
    ...
    fn run_subprocess(
        ...
        std::thread::spawn(move || {
            ...
            let _ = rt.block_on(session.run_subprocess(
                &command, args_ref, duration, env_ref, use_session_env_vars, banner_ref,
            ));
            *session_arc.lock().unwrap() = Some(session);
            let guard = session_arc.lock().unwrap();
            if let Some(s) = guard.as_ref() {
    ...
    fn cancel_action(&self, time_limit: Option<f64>, mark_action_failed: Option<bool>) -> PyResult<()> {
        ...
        let mut guard = self.session.lock().unwrap();
        match guard.as_mut() {
            Some(session) => session.cancel_action(duration, mark_action_failed.unwrap_or(false))
