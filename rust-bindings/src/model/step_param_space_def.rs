...
fn int_or_fs_to_py<'py>(
    ...
    match v {
        ...
        IntOrFormatString::FormatString(fs) => {
            Ok(PyFormatString { inner: fs.clone() }.into_pyobject(py)?.into_any())
        }
    }
}
...
/// `FormatString`).
fn float_range_item_to_py<'py>(
    py: Python<'py>,
    v: &FloatRangeItem,
) -> PyResult<Bound<'py, PyAny>> {
    match v {
        FloatRangeItem::Float(f) => Ok(f.into_pyobject(py)?.into_any()),
        FloatRangeItem::FormatString(fs) => {
            Ok(PyFormatString { inner: fs.clone() }.into_pyobject(py)?.into_any())
        }
    }
}
...
impl PyChunksDefinition {
    ...
    fn default_task_count_camel<'py>(
        &self,
        py: Python<'py>,
    ) -> PyResult<Bound<'py, PyAny>> {
...
impl PyIntTaskParameterDefinition {
    ...
    fn __repr__(&self) -> String {
        format!("IntTaskParameterDefinition(name={:?})", self.inner.name.as_str())
    }
}
...
impl PyFloatTaskParameterDefinition {
    ...
    fn __repr__(&self) -> String {
        format!("FloatTaskParameterDefinition(name={:?})", self.inner.name.as_str())
    }
}
...
impl PyChunkIntTaskParameterDefinition {
    ...
    fn chunks(&self) -> PyChunksDefinition {
        PyChunksDefinition { inner: self.inner.chunks.clone() }
    }
...
fn task_param_def_to_py<'py>(
    ...
    match def {
        TaskParameterDefinition::INT(p) => {
            Bound::new(py, PyIntTaskParameterDefinition { inner: p.clone() })
                .map(|b| b.into_any())
        }
        ...
        TaskParameterDefinition::PATH(p) => {
            Bound::new(py, PyPathTaskParameterDefinition { inner: p.clone() })
                .map(|b| b.into_any())
        }
...
impl PyStepParameterSpaceDefinition {
    ...
    /// `ChunkIntTaskParameterDefinition`.
    #[getter]
    fn task_parameter_definitions<'py>(
        &self,
        py: Python<'py>,
