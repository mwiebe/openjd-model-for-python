...
use openjd_model::template::{
    ...
    JobParameterDefinition, JobPathParameterDefinition, JobRangeExprParameterDefinition,
    JobStringParameterDefinition,
};
...
impl PyJobIntParameterDefinition {
    ...
    fn allowed_values(&self) -> Option<Vec<i64>> {
        self.inner.allowed_values.as_ref().map(|v| v.iter().map(|f| f.0).collect())
    }
...
impl PyJobListIntParameterDefinition {
    ...
    fn default(&self) -> Option<Vec<i64>> {
        self.inner.default.as_ref().map(|v| v.iter().map(|f| f.0).collect())
    }
...
impl PyJobListBoolParameterDefinition {
    ...
    fn default(&self) -> Option<Vec<bool>> {
        self.inner.default.as_ref().map(|v| v.iter().map(|b| b.0).collect())
    }
...
pub(crate) fn job_param_def_to_py<'py>(
    ...
    match def {
        ...
        JobParameterDefinition::INT(p) => {
            Bound::new(py, PyJobIntParameterDefinition { inner: p.clone() })
                .map(|b| b.into_any())
        }
        JobParameterDefinition::FLOAT(p) => {
            Bound::new(py, PyJobFloatParameterDefinition { inner: p.clone() })
                .map(|b| b.into_any())
        }
        JobParameterDefinition::PATH(p) => {
            Bound::new(py, PyJobPathParameterDefinition { inner: p.clone() })
                .map(|b| b.into_any())
        }
        JobParameterDefinition::BOOL(p) => {
            Bound::new(py, PyJobBoolParameterDefinition { inner: p.clone() })
                .map(|b| b.into_any())
        }
