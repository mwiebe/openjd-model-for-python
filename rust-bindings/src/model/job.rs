...
impl PyJob {
    ...
    fn extensions(&self) -> Option<Vec<String>> {
        self.inner.extensions.as_ref().map(|exts| exts.iter().map(|e| e.as_str().to_string()).collect())
    }

    #[getter]
    fn steps(&self) -> Vec<PyStep> {
        self.inner.steps.iter().map(|s| PyStep { inner: s.clone() }).collect()
    }

    #[getter]
    fn parameters(&self) -> HashMap<String, PyJobParameter> {
        self.inner.parameters.iter().map(|(k, v)| {
            (k.clone(), PyJobParameter { inner: v.clone() })
        }).collect()
    }
    ...
    fn job_environments(&self) -> Option<Vec<PyEnvironment>> {
        self.inner.job_environments.as_ref().map(|envs| {
            envs.iter().map(|e| PyEnvironment { inner: e.clone() }).collect()
        })
    }
...
impl PyStep {
    ...
    fn script(&self) -> PyStepScript {
        PyStepScript { inner: self.inner.script.clone() }
    }
    ...
    fn parameter_space(&self) -> Option<PyStepParameterSpace> {
        self.inner.parameter_space.as_ref().map(|ps| PyStepParameterSpace { inner: ps.clone() })
    }
    ...
    fn resolved_symtab(&self) -> Option<crate::expr::PySymbolTable> {
        self.inner.resolved_symtab.as_ref().and_then(|st| st.to_symtab(openjd_expr::path_mapping::PathFormat::host()).ok()).map(|st| crate::expr::PySymbolTable { inner: st })
    }
    ...
    fn dependencies(&self) -> Option<Vec<PyStepDependency>> {
        self.inner.dependencies.as_ref().map(|deps| {
            deps.iter().map(|d| PyStepDependency { inner: d.clone() }).collect()
        })
    }
...
impl PyStepScript {
    ...
    fn embedded_files(&self) -> Option<Vec<PyEmbeddedFile>> {
        self.inner.embedded_files.as_ref().map(|files| {
            files.iter().map(|f| PyEmbeddedFile { inner: f.clone() }).collect()
        })
    }
...
impl PyAction {
    ...
    fn command(&self) -> crate::expr::PyFormatString {
        crate::expr::PyFormatString { inner: self.inner.command.clone() }
    }

    #[getter]
    fn args(&self) -> Option<Vec<crate::expr::PyFormatString>> {
        self.inner.args.as_ref().map(|a| a.iter().map(|fs| crate::expr::PyFormatString { inner: fs.clone() }).collect())
    }
...
impl PyEnvironment {
    ...
    fn variables(&self) -> Option<HashMap<String, String>> {
        self.inner.variables.as_ref().map(|vars| {
            vars.iter().map(|(k, v)| (k.clone(), v.raw().to_string())).collect()
        })
    }
...
#[cfg_attr(feature = "stub-gen", gen_stub_pyclass(module = "openjd._openjd_rs"))]
#[pyclass(module = "openjd.model._v1.job", name = "EnvironmentScript", from_py_object)]
#[derive(Clone)]
...
#[cfg_attr(feature = "stub-gen", gen_stub_pyclass(module = "openjd._openjd_rs"))]
#[pyclass(module = "openjd.model._v1.job", name = "EnvironmentActions", from_py_object)]
#[derive(Clone)]
...
#[cfg_attr(feature = "stub-gen", gen_stub_pyclass(module = "openjd._openjd_rs"))]
#[pyclass(module = "openjd.model._v1.job", name = "StepParameterSpace", from_py_object)]
#[derive(Clone)]
...
#[cfg_attr(feature = "stub-gen", gen_stub_pyclass(module = "openjd._openjd_rs"))]
#[pyclass(module = "openjd.model._v1.job", name = "StepDependency", from_py_object)]
#[derive(Clone)]
...
#[cfg_attr(feature = "stub-gen", gen_stub_pyclass(module = "openjd._openjd_rs"))]
#[pyclass(module = "openjd.model._v1.job", name = "CancelationMode", from_py_object)]
#[derive(Clone)]
...
impl PyCancelationMode {
    ...
    fn notify_period_in_seconds(&self) -> Option<String> {
        match &self.inner {
            job::CancelationMode::NotifyThenTerminate { notify_period_in_seconds } => {
                notify_period_in_seconds.as_ref().map(|t| t.raw().to_string())
            }
            _ => None,
        }
    }
